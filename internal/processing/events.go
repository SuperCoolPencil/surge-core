...
func (mgr *LifecycleManager) StartEventWorker(ch <-chan interface{}) {
	for msg := range ch {
		...
		case events.DownloadCompleteMsg:
			...
			if err := finalizeCompletedFile(destPath); err != nil {
				...
				if settings := mgr.GetSettings(); settings != nil && settings.General.DownloadCompleteNotification {
					if mgr.Notify != nil {
						mgr.Notify(fmt.Sprintf("Download failed: %s", filename), msg)
					}
				}
				break
			}
			...
			if settings := mgr.GetSettings(); settings != nil && settings.General.DownloadCompleteNotification {
				...
				if m.Elapsed.Seconds() <= 0 {
					if mgr.Notify != nil {
						mgr.Notify(title, "Download complete!")
					}
				} else {
					if mgr.Notify != nil {
						mgr.Notify(title, fmt.Sprintf("Download complete in %s (%.2f MB/s)", m.Elapsed.Truncate(time.Second), avgSpeed/float64(types.MB)))
					}
				}
			}
		...
		case events.BatchProgressMsg, events.ProgressMsg:
			// Progress ticks are intentionally transient.
		}
	}
}
